...
func TestAllowCharsetIntegration(t *testing.T) {
	...
	require.Empty(t, violations, "utf8mb4 charset should be allowed by default")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "latin1 charset should not be allowed")
	require.Contains(t, violations[0].Message, "charset_test_disallowed")
	...
	require.NotEmpty(t, violations, "Adding column with latin1 charset should not be allowed")
	require.Contains(t, violations[0].Message, "latin1")
	require.Contains(t, violations[0].Message, "disallowed")
	...
	violations = linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Adding column with utf8mb4 charset should be allowed")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Modifying column to latin1 charset should not be allowed")
	require.Contains(t, violations[0].Message, "latin1")
	require.Contains(t, violations[0].Message, "disallowed")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Changing column to latin1 charset should not be allowed")
	require.Contains(t, violations[0].Message, "latin1")
	require.Contains(t, violations[0].Message, "disallowed")
...
func TestAllowEngineIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "InnoDB engine should be allowed by default")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "MyISAM engine should not be allowed")
	require.Contains(t, violations[0].Message, "engine_test_disallowed")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Altering to MyISAM engine should not be allowed")
	require.Contains(t, violations[0].Message, "engine_test_allowed")
	require.Contains(t, violations[0].Message, "MyISAM")
	...
	violations = linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Altering to InnoDB engine should be allowed")
...
func TestHasFKIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "Table without foreign keys should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Table with foreign key should be detected")
	require.Contains(t, violations[0].Message, "fk_test_child")
...
func TestHasFloatIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "Table without float columns should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Table with FLOAT column should be detected")
	require.Contains(t, violations[0].Message, "float_test_with_float")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Table with DOUBLE column should be detected")
	require.Contains(t, violations[0].Message, "float_test_with_double")
...
func TestInvisibleIndexIntegration(t *testing.T) {
	...
	violations := linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Dropping index without existing table info should generate warning")
	require.Contains(t, violations[0].Message, "idx_name")
	...
	require.NotEmpty(t, violations, "Dropping visible index should generate warning")
	require.Contains(t, violations[0].Message, "idx_name")
	require.Contains(t, violations[0].Message, "invisible")
	...
	alterStmt = statement.MustNew("ALTER TABLE invisible_test DROP INDEX idx_name")
	violations = linter.Lint([]*statement.CreateTable{ct}, alterStmt)
	require.Empty(t, violations, "Dropping invisible index should not generate warning")
...
func TestMultipleAlterIntegration(t *testing.T) {
	...
	violations := linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Single ALTER statement should pass")
	...
	require.NotEmpty(t, violations, "Multiple ALTER statements should be detected")
	require.Contains(t, violations[0].Message, "test_table")
}
...
func TestNameCaseIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "Lowercase table and column names should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Mixed case names should be detected")
...
func TestPrimaryKeyTypeIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "BIGINT UNSIGNED primary key should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "INT primary key should generate error")
	require.Contains(t, violations[0].Message, "id")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "VARBINARY primary key should pass")
...
func TestUnsafeIntegration(t *testing.T) {
	...
	alterStmt := statement.MustNew("ALTER TABLE test_table ADD COLUMN new_col INT")
	violations := linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Safe ALTER statement should pass")
	...
	require.NotEmpty(t, violations, "DROP COLUMN should be detected as unsafe")
	require.Contains(t, violations[0].Message, "DROP")
}
...
func TestZeroDateIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "Table without zero date defaults should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Table with zero date default should be detected")
	require.Contains(t, violations[0].Message, "created_at")
	require.Contains(t, violations[0].Message, "zero default value")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Adding column with zero date default should be detected")
	require.Contains(t, violations[0].Message, "legacy_date")
	require.Contains(t, violations[0].Message, "zero default value")
	...
	violations = linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Adding column with valid default should pass")
	...
	violations = linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Adding NOT NULL date column without default is permitted (at least for now)")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Modifying column to have zero date default should be detected")
	require.Contains(t, violations[0].Message, "created_at")
	require.Contains(t, violations[0].Message, "zero default value")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Changing column to have zero date default should be detected")
	require.Contains(t, violations[0].Message, "created_at")
	require.Contains(t, violations[0].Message, "zero default value")
	...
	violations = linter.Lint(nil, alterStmt)
	require.NotEmpty(t, violations, "Adding column with zero DATE default should be detected")
	require.Contains(t, violations[0].Message, "zero_date")
	require.Contains(t, violations[0].Message, "zero default value")
	...
	violations = linter.Lint(nil, alterStmt)
	require.Empty(t, violations, "Adding nullable column with NULL default should pass")
...
func TestAutoIncCapacityIntegration(t *testing.T) {
	...
	violations := linter.Lint([]*statement.CreateTable{ct}, nil)
	require.Empty(t, violations, "Table with low auto_increment should pass")
	...
	violations = linter.Lint([]*statement.CreateTable{ct}, nil)
	require.NotEmpty(t, violations, "Table with high auto_increment should generate warning")
