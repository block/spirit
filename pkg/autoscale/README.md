...
It is a bar on the *controller*, not on the derivations below it. `ReadBounds`, `WriteStart` and `FlushBounds` are pure functions of the instance size and are defined at every size, and a small instance is where they matter most: the threads throttler hard-stops the copy when the running-thread count passes a threshold derived from the same vCPU count, so pools sized from a constant instead can hold more threads than that threshold allows — and with no controller to shed them, the copy throttles on its own workers at any table size. `TestSmallInstancePoolsFitTheThreshold` in `pkg/throttler`, which can see both halves of that comparison, pins the fit.
...
- **`ReadBounds`** derives the read side's starting size and ceiling from the instance vCPU count — `max(2, ceil((vCPUs - VCPUReserve) / 4))` up to `ceil(vCPUs / 2)`, so a `4xlarge` reads with 4 workers and may grow to 8, a `24xlarge` with 24 growing to 48. This is where the asymmetry with the write side lives: write threads mostly sit parked on a redo-log flush, so a count above the vCPU count is not oversubscription (and the redo-aware signal excludes those waiters), whereas a read thread scanning an in-buffer-pool table is pure CPU and does compete with the application for cores. The read side therefore starts at about a quarter of the instance and earns its way up through the band.
  ...
  Both bounds come from the instance rather than from `--threads`. When the flag is passed against an Aurora target, the migration runner ignores `--threads` and `--write-threads` entirely: a controller told to find the right size should not also be told where to stop, and those flags are usually left at their defaults. Below `MinVCPUs` the starting sizes still come from the instance and only the ceiling goes away, since nothing will move the pool off its start. [docs/migrate.md](../../docs/migrate.md#enable-experimental-autoscaling) has the sizing worked out per instance type.
