...
2. **Spirit's normalization rules** handle the canonicalizations the parser does *not* — each mirrors something MySQL does when storing the table:
   ...
   | `booleanKeywordDefaultNormalizer` | folds a bare `TRUE`/`FALSE` keyword `DEFAULT` to the `1`/`0` MySQL stores, bare on a numeric column and quoted on a string one (`BOOLEAN NOT NULL DEFAULT FALSE` → `tinyint(1) NOT NULL DEFAULT '0'`; `varchar(8) NOT NULL DEFAULT FALSE` → `DEFAULT '0'`). Folds on the integer types, unscaled `decimal`, `double`/`float`, `varchar`/`char` and `varbinary`. Left alone where the keyword is stored as something else: scaled `decimal` pads to its scale, `YEAR` reads it as a year, `binary` pads to the column width with NULs, `bit` stores a bit literal, and on `enum`/`set` the keyword names a member index rather than a value |
