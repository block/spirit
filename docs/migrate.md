...
| Field | Meaning |
| --- | --- |
| `%` | Rows settled out of the estimated total. The total comes from table statistics, so the percentage is approximate and not a row count you should reconcile against. After a resume from checkpoint on a table with an auto_increment key, the count restarts at the rows settled since the restart while the total is still the whole table (the checkpoint keeps the copy position, not a row count), so `%` restarts low and finishes short of 100% while `eta` continues from where the copy left off. |
