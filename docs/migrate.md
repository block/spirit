...
| `n/m` | The figures the percentage is derived from: rows settled so far and the estimated table cardinality, the same numbers the status API reports per table and as `Copy`. After a resume from checkpoint, `n` counts the rows settled since the restart on a table with an auto_increment key (the checkpoint keeps the copy position, not a row count), so `%` restarts low while `eta` continues from where the copy left off. |
