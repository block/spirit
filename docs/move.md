...
The explicit budget must cover `--threads` plus six connections of checksum headroom. During setup, read concurrency and its autoscaling ceiling are fitted to the pool, accounting for table statistics queries and checksum snapshots. Write workers may wait for a connection. This is a per-pool limit, not a total across the move.
