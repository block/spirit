...
# Keep stderr out of the package list, and consult the exit status on its own,
# so a `go list` failure cannot be mistaken for a clean graph: an unresolvable
# module exits non-zero while still printing most of the graph.
stderr_file=$(mktemp)
trap 'rm -f "$stderr_file"' EXIT
if ! deps=$(go list -deps -test ./... 2>"$stderr_file"); then
...
if ! printf '%s\n' "$deps" | grep -qx "github.com/block/mysql"; then
